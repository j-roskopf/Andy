...
fun quarterTurnForRoll(roll: Float): Int {
...
data class EmulatorImageFormat(val maxSize: Int, val mmapHandle: String? = null)
data class EmulatorImage(
    val width: Int,
    val height: Int,
...
data class EmulatorTouch(val x: Int, val y: Int, val pressure: Int)
data class EmulatorTouchEvent(val touches: List<EmulatorTouch>, val display: Int = 0)
data class EmulatorKeyEvent(val eventType: Int = KEY_EVENT_KEYDOWN, val key: String = "", val text: String = "")

data class PhysicalModelRequest(val physicalType: Int, val values: List<Float>)

data class PhysicalModelValue(val physicalType: Int, val values: List<Float>)
