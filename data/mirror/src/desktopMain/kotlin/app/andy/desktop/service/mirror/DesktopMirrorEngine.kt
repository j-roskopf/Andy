...
import app.andy.domain.parseWmUserRotation
...
import app.andy.service.forRemoteTunnel
...
data class EmulatorDisplaySize(val width: Int, val height: Int)
private data class LockedEmulatorRotation(val quarterTurn: Int, val landscape: Boolean)
data class EmulatorTouchPoint(val x: Int, val y: Int)
data class ScrcpySessionFrame(val width: Int, val height: Int, val clientResized: Boolean)

/** Extracts the big-endian payload length from scrcpy's 8-byte PTS/flags + 4-byte-size header. */
fun scrcpyFramePayloadSize(header: ByteArray): Int {
...
fun scrcpyFrameIsSession(header: ByteArray): Boolean {
    require(header.size == SCRCPY_FRAME_HEADER_BYTES) { "Expected a $SCRCPY_FRAME_HEADER_BYTES-byte scrcpy frame header" }
    return header[0].toInt() and 0x80 != 0
}

fun scrcpySessionFrame(header: ByteArray): ScrcpySessionFrame {
...
fun emulatorRgb888ToArgb(width: Int, height: Int, rgb: ByteBuffer): IntArray {
...
fun isScrcpyForwardDummyByte(value: Int): Boolean = value == 0
 ...
 */
fun remoteScrcpyServerSizeMatches(localByteCount: Long, remoteSizeStdout: String): Boolean {
...
fun videoCodecOptionsForDevice(
 ...
 */
fun effectiveEmulatorTouchDisplaySize(
    frame: MirrorFrame,
    displaySize: EmulatorDisplaySize?,
): EmulatorDisplaySize? {
...
class DesktopMirrorEngine(
     ...
     */
    private val forwardBridge: app.andy.service.AdbForwardBridge? = null,
    /**
    ...
    private suspend fun runNativeVideoLoop(adb: String, serial: String, scrcpyServer: File, config: MirrorVideoConfig) = withContext(Dispatchers.IO) {
        ...
        val command = listOf(
            ...
            "video_codec=${config.codec.ifBlank { "h264" }}",
            "log_level=info",
        ) + codecOptions
        var frameNumber = 0L
        var fpsWindowStartedAt = System.nanoTime()
