...
fun discoverAgentSkills(agent: AgentKind, directory: String?): List<AgentSkill> {
...
fun discoverKnownAgentSkillNames(directory: String?): Set<String> = AgentKind.entries
    .flatMap { discoverAgentSkills(it, directory) }
    .mapTo(linkedSetOf()) { it.name.normalizedAgentCommandName() }

fun String.normalizedAgentCommandName(): String = trim().trimStart('/', '$').lowercase()
