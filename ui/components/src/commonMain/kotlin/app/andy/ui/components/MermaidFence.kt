...
fun isMermaidFenceLanguage(language: String?): Boolean {
    val token = language
        ?.trim()
        ?.substringBefore(' ')
        ?.substringBefore('{')
...
fun mermaidSvgIntrinsicSize(svg: String, fallbackWidth: Float = 800f, fallbackHeight: Float = 400f): Pair<Float, Float> {
