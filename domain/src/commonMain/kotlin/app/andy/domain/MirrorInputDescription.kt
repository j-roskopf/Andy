...
fun mirrorInputBugText(input: MirrorInput, accessibilityRoot: AccessibilityNode?): Pair<String, String?> = when (input) {
...
fun mirrorTapBugText(x: Int, y: Int, accessibilityRoot: AccessibilityNode?): Pair<String, String?> {
...
fun mirrorSwipeBugText(startX: Int, startY: Int, endX: Int, endY: Int, durationMillis: Int): Pair<String, String?> {
    val dx = endX - startX
    val dy = endY - startY
...
fun AccessibilityNode.accessibilityStateSummary(): String? {
...
fun androidKeyLabel(keyCode: Int): String? = when (keyCode) {
    24 -> "Volume up"
    25 -> "Volume down"
    else -> null
}
