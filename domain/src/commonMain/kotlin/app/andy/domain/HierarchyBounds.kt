...
fun offsetBoundsY(bounds: String?, deltaY: Int): String? {
...
fun boundsString(left: Int, top: Int, right: Int, bottom: Int): String = "[$left,$top][$right,$bottom]"
 ...
 */
fun needsScrollTranslation(parsed: List<Int>, viewport: List<Int>, scrollY: Int): Boolean {
...
fun AccessibilityNode.scrollOffsetY(): Int =
...
fun AccessibilityNode.nearestScrollableAncestor(root: AccessibilityNode): AccessibilityNode? {
    val path = pathFromRoot(root) ?: return null
    return path.dropLast(1).lastOrNull { it.scrollable }
}

fun AccessibilityNode.pathFromRoot(root: AccessibilityNode): List<AccessibilityNode>? {
...
fun boundsTop(bounds: String?): Int? = parseBounds(bounds)?.getOrNull(1)
