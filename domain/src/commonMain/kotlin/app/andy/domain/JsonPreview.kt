...
data class JsonPreviewRow(
    val node: JsonPreviewNode,
    val depth: Int,
    val text: String,
)

fun parseJsonBodyPreview(body: String?): JsonPreviewNode? {
...
fun flattenJsonPreview(root: JsonPreviewNode, expandedKeys: Map<String, Boolean>): List<JsonPreviewRow> {
...
fun jsonPreviewRowText(node: JsonPreviewNode, expanded: Boolean): String {
...
class JsonPreviewParser(private val source: String) {
    private var index = 0
...
fun escapePathSegment(value: String): String {
...
fun quoteJsonPreview(value: String): String {
    return buildString {
        append('"')
        value.forEach { char ->
            when (char) {
