...
fun String.composerCommandName(): String = trim().trimStart('/', '$')

/** Formats provider and native commands using Andy's shared `/name` convention. */
fun String.composerCommandToken(): String = "/${composerCommandName()}"
...
private val cliSkillHintsMarker = "\n\nUse these local skill instructions before responding:\n"

/** Strips CLI-only skill path hints Andy appends before sending a turn to a provider. */
fun stripCliSkillHints(text: String): String {
    val markerIndex = text.indexOf(cliSkillHintsMarker)
    return if (markerIndex >= 0) text.substring(0, markerIndex).trimEnd() else text
}

/** True when the composer text is only slash-selected skills (no additional user prose). */
fun isSkillOnlyMessage(text: String, skills: List<AgentSkill>): Boolean {
    if (skills.isEmpty()) return false
    val tokens = text.trim().split(Regex("\\s+")).filter { it.isNotBlank() }
    if (tokens.isEmpty()) return true
    val skillTokens = skills.mapTo(linkedSetOf(String.CASE_INSENSITIVE_ORDER)) { "/${it.name}" }
    return tokens.all { token -> skillTokens.contains(token) }
}
