...
fun contextualAiActionsEnabled(services: AndyServices): Boolean =
...
@Composable
fun rememberContextualAiActionState(): ContextualAiActionState = remember { ContextualAiActionState() }

/** Renders the confirmation sheet for whichever action [state] currently holds, if any. */
@Composable
fun ContextualAiActionHost(services: AndyServices, state: ContextualAiActionState) {
...
@Composable
fun ExplainActionButton(label: String, enabled: Boolean = true, onClick: () -> Unit) {
...
data class InvestigationEventLocation(
 ...
 */
suspend fun findInvestigationEvent(
    bugs: BugService,
    key: String,
    value: String,
    searchDepth: Int = 5,
 ...
 */
suspend fun findLatestInvestigationEvent(
    bugs: BugService,
